...
def getMeanTF(image_np):
    image_tf = tf.convert_to_tensor(image_np)

    height, width, color = image_tf.shape
    mean_filter = tfa.image.mean_filter2d(image_tf, (4,4))

    strided_filter = tf.strided_slice(mean_filter, [0,0,0], [height,width,3], [4,4,1])
    #strided_filter = strided_filter.astype(int)
    return strided_filter
...
def main():
    ...
    while True:
        try:
            ...
            #image_tf = tf.convert_to_tensor(mean_np)
            #height, width, color = image_tf.shape
            #strided_filter = tf.strided_slice(image_tf, [0,0,0], [height,width,3], [4,4,1])
